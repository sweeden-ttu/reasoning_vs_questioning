"""Train & Benchmark Tripartite Baseline over 6 Months & 8 Opponents.

Runs 6 full 30-day seasons (4,320 turns per opponent x 2 seat positions = 8,640 turns per opp)
across 8 reference opponents:
...
Total simulated match turns: 8 opps x 6 seasons x 2 seats x 720 = 69,120 turns.
Indexes strategic state transitions, market moves, and counter-tactics into
...
logger = logging.getLogger("baseline_trainer_8opp")
...
class BaselineTrainer8Opponents:
    """Orchestrator for 6-month x 8-opponent competitive baseline training."""
    ...
    def run_match_season(
        ...
        """Execute one 30-day (720-step) season against opponent, indexing transitions."""
        ...
        while not done and steps < self.turns_per_season:
            ...
            # Index daily state summaries and decisive high-reward turns
            if steps % 24 == 0 or reward > 0.5:
                tag = f"{opp_name}_m{season_idx+1}_d{steps//24}"
                self.vector_bank.add_entry(
                    obs=obs_agent,
                    action=act_agent,
                    strategic_value=float(reward),
                    archetype_tag=opp_name,
                    dialogue_response=f"Strategic match state vs {opp_name} in season {season_idx+1}",
                )
    ...
    def train_baseline(self) -> Dict[str, Any]:
        """Run complete 6-month x 8-opponent baseline training across both seats."""
        opponents = self.load_opponents()
        print("=== Starting Baseline Training: 6 Months (6 Seasons x 720 turns) across 8 Opponents ===")
        ...
        for opp_name, opp_fn in opponents.items():
            ...
            for season in range(self.seasons_per_opp):
                # Play both as Seat 0 and Seat 1
                for seat in (0, 1):
                    seed = self.base_seed + (len(all_match_results) * 19)
        ...
        dummy_query_obs = {"day": 15, "hour": 8, "farms": [{"money": 15000.0}, {"money": 12000.0}]}
        ...
        print("=== 6-Month 8-Opponent Baseline Training Complete ===")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train baseline agent over 6 months and 8 opponents")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT_DIR / "experiments" / "baseline_6months_8opponents",
    ...
    trainer = BaselineTrainer8Opponents(
